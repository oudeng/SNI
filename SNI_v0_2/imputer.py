# imputer.py - SNI Imputer (v0.2 Optimized)
# Statistical-Neural Interaction for Missing Data Imputation
# Fixed: Categorical variables now properly encoded for training (2026-01-15)
...
class SNIConfig:
    """
    SNI Configuration - High-Performance defaults for v0.2.1
...
class SNIImputer:
    """
    Statistical-Neural Interaction (SNI) Imputer - v0.2
        ...
        for epoch in range(self.cfg.epochs):
            ...
            for batch in train_loader:
                ...
                if self.cfg.variant == "HardPrior":
                    ...
                    A_vec_heads = A_heads.mean(dim=1)
                    loss_prior = model.compute_prior_loss(A_vec_heads, P_t, lambdas, alpha)
            ...
            with torch.no_grad():
                if len(val_idx) > 0:
                    ...
                    if self.cfg.variant == "HardPrior":
                        lambdas_eff = torch.full_like(lambdas_v, float(self.cfg.hard_prior_lambda))
                        A_vec_heads_v = A_heads_v.mean(dim=1)
                        loss_prior_v = model.compute_prior_loss(A_vec_heads_v, P_t, lambdas_eff, alpha)
                    elif self.cfg.variant == "NoPrior":
                        loss_prior_v = torch.tensor(0.0, device=device)
                    else:
                        A_vec_heads_v = A_heads_v.mean(dim=1)
                        loss_prior_v = model.compute_prior_loss(A_vec_heads_v, P_t, lambdas_v, alpha)
