...
def _load_baseline_summaries(project_root: Path, mechanisms: List[str]) -> List[pd.DataFrame]:
    ...
    dir_patterns = [
        "results_baselines_main",       # MCAR, MAR (project root layout)
        ...
        "baselines_main",               # run_all_experiments.py layout
        "baselines_mnar",
        "baselines_deep",
        "baselines_new",
        "agg_baselines_main",           # aggregated output dirs
        "agg_baselines_mnar",
        "agg_baselines_deep",
        "agg_baselines_new",
    ]
...
def _load_sni_results(project_root: Path, mechanisms: List[str]) -> List[pd.DataFrame]:
    ...
    frames: List[pd.DataFrame] = []
    sni_dirs = [
        "results_sni_main",        # MCAR, MAR (project root layout)
        "results_sni_mnar",        # MNAR
        "results_sni_v03_main",    # v0.3 MCAR, MAR
        "sni_v03_main",            # run_all_experiments.py layout
        "sni_mnar",
    ]
    # Also check aggregated output dirs for summary_all.csv (has per-seed rows)
    sni_agg_dirs = [
        "agg_sni_v03_main",
        "agg_sni_mnar",
        "agg_sni_v03_ablation_lambda",
    ]
    mech_upper = {m.upper() for m in mechanisms}

    # --- Load from aggregated summary_all.csv (faster, already parsed) ---
    for agg_dir_name in sni_agg_dirs:
        agg_dir = project_root / agg_dir_name
        if not agg_dir.exists():
            continue
        for csv_name in ["summary_all.csv"]:
            p = agg_dir / csv_name
            if not p.exists():
                continue
            try:
            ...
            if len(df) > 0:
                df["_source_path"] = str(p)
                frames.append(df)
                print(f"  [load] {p}: {len(df)} rows (SNI agg)")

    # --- Load from per-experiment folders (original layout) ---
