...
# ======================== TL;DR ========================
# Smoke test (validate environment, ~5 min)
# python scripts/paper_quickstart.py --smoke-test --outdir results_quick
#
# Run ALL paper experiments (one command, supports resume)
# python scripts/run_all_experiments.py --outdir results_all --skip-existing --n-jobs-cpu 8 --n-jobs-gpu 1
#
...
#
# Post-process only (tables + figures from existing results)
# python scripts/paper_quickstart.py --postprocess-only --sni-results ... --baseline-results ... --outdir results_paper
#
# For detailed docs, see docs/full_reproduction.md
# =======================================================
...
#                                   : Categorical loss reweighting
#   --cat-lr-mult <float>          : Learning rate multiplier for categorical
#                                     heads (default: 1.0)
#   --lambda-mode learned|fixed    : Per-head lambda mode
...
# ========================|=================================|===========|========
# TOTAL (all stages)      | run_all_experiments.py          | ~24-48h   | auto
#
...
#-------------------------------------------------------------------------------
# 8.1 Run ALL Experiments (recommended for full reproduction)
#-------------------------------------------------------------------------------
# This single command runs ALL stages: SNI, baselines, ext experiments,
# sanity checks, and final aggregation with table/figure generation.
...
#   ├── agg_sni_v03_main/          # Stage 7a: Aggregated per-group summaries
#   ├── agg_baselines_main/        #   (one agg_* dir per result group)
#   ├── agg_baselines_deep/        #   supports fallback to summary_all_runs.csv
#   ├── ...                        #
...
#-------------------------------------------------------------------------------
# --skip-existing automatically detects completed runs and skips them.
...
#-------------------------------------------------------------------------------
# 8.3 Run Specific Stages Only
#-------------------------------------------------------------------------------
# Stage 1: SNI experiments only
python scripts/run_all_experiments.py --outdir results_all --stages 1 --skip-existing

# Stages 1-2: SNI + classic baselines (no GPU needed)
python scripts/run_all_experiments.py --outdir results_all --stages 1 2 --use-gpu false --skip-existing

# Stage 7 only: re-aggregate and regenerate tables/figures
python scripts/run_all_experiments.py --outdir results_all --stages 7

#-------------------------------------------------------------------------------
# 8.4 Dry Run (preview commands)
#-------------------------------------------------------------------------------
python scripts/run_all_experiments.py --outdir results_all --dry-run
