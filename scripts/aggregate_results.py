...
from scripts.reporting_constants import ALGO_ORDER_MAIN
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Profile spec e.g. configs/paper_profiles.yaml:main",
    )
    ap.add_argument(
        "--include-algos",
        type=str,
        default=None,
        help="Comma-separated algo filter (overrides profile)",
    )
    ap.add_argument(
        "--exclude-algos",
        type=str,
        default=None,
        help="Comma-separated algo exclusion list",
    )
    args = ap.parse_args()
    ...
    if len(files) == 0:
        # Fallback: check for pre-built summary_all_runs.csv / summary_all.csv
        fallback_names = ["summary_all_runs.csv", "summary_all.csv"]
        fallback_found = None
        ...
        if fallback_found is not None:
            print(f"[INFO] No per-experiment metrics found; using pre-built {fallback_found.name}")
            files = [fallback_found]
        else:
    ...
    for fp in files:
        ...
        if "exp_id" in df.columns:
            if len(df) == 1:
                # Single-row file: apply parsed metadata to entire row
                parsed = _parse_exp_id(df.loc[df.index[0], "exp_id"])
                for k, v in parsed.items():
                    if k not in df.columns:
                        df[k] = v
            else:
                # Multi-row file (e.g. summary_all_runs.csv): parse per-row
                missing_cols = [k for k in ("dataset", "mechanism", "rate", "exp_body", "exp_prefix")
                                if k not in df.columns]
                if missing_cols:
                    parsed_series = df["exp_id"].map(_parse_exp_id)
                    for k in missing_cols:
                        df[k] = parsed_series.map(lambda x, _k=k: x.get(_k))
                    if "seed_parsed" not in df.columns and "seed" not in df.columns:
                        df["seed"] = parsed_series.map(lambda x: x.get("seed_parsed"))
        if "rate" in df.columns and "rate_float" not in df.columns:
            df["rate_float"] = df["rate"].map(_rate_to_float)
        rows.append(df)
    ...
    # ---- profile / algo filtering ---- #
    profile = load_profile(args.profile)
    if "algo" in all_df.columns:
        all_df = filter_dataframe(all_df, profile)
    # CLI --include-algos overrides profile's include_algos
    if args.include_algos:
        keep = [a.strip() for a in args.include_algos.split(",") if a.strip()]
        all_df = all_df[all_df["algo"].isin(keep)]
    if args.exclude_algos:
        drop = [a.strip() for a in args.exclude_algos.split(",") if a.strip()]
        all_df = all_df[~all_df["algo"].isin(drop)]
    all_df = all_df.reset_index(drop=True)

    # Output filename prefix from profile
    out_prefix = profile.get("output_prefix", "")
    def _out(name: str) -> Path:
        """Return output path, prepending profile prefix if set."""
        if out_prefix:
            return outdir / f"{out_prefix}_{name}"
        return outdir / name
    ...
    # Save run-level
    all_df.to_csv(_out("summary_all.csv"), index=False)
    ...
    agg_df.to_csv(_out("summary_agg.csv"), index=False)
    ...
    overall_df.to_csv(_out("summary_overall_by_algo.csv"), index=False)
    ...
    if not ranks_df.empty:
        ranks_df.to_csv(_out("summary_ranks.csv"), index=False)
        ranks_agg = ranks_df.groupby(["metric", "algo"], as_index=False).agg(
            n_tasks=("rank", "count"),
            avg_rank=("rank", "mean"),
            std_rank=("rank", "std"),
        )
        ranks_agg.to_csv(_out("summary_ranks_agg.csv"), index=False)
    else:
    ...
    if not wins_df.empty:
        ...
        wins_agg.to_csv(_out("summary_wins.csv"), index=False)
    ...
    if not rel_df.empty:
        rel_df.to_csv(_out("summary_rel_to_ref.csv"), index=False)
