...
from scripts.reporting_constants import ALGO_ORDER_MAIN, ALGO_RENAME_MAP
...
def main():
    ...
    ap.add_argument("--include-algos", type=str, default=None, help="Comma-separated algo filter (overrides profile)")
    ap.add_argument("--exclude-algos", type=str, default=None, help="Comma-separated algo exclusion list")
    ap.add_argument("--algo-order", type=str, default=None, help="Comma-separated algo column order")
    args = ap.parse_args()
    ...
    if args.include_algos:
        keep = [a.strip() for a in args.include_algos.split(",")]
        df = df[df["algo"].isin(keep)]
    if args.exclude_algos:
        drop = [a.strip() for a in args.exclude_algos.split(",")]
        df = df[~df["algo"].isin(drop)]

    # algo ordering
    algo_order = (
        [a.strip() for a in args.algo_order.split(",")]
        if args.algo_order
        else ALGO_ORDER_MAIN
    )
    order_map = {a: i for i, a in enumerate(algo_order)}
    df = df.sort_values("algo", key=lambda s: s.map(order_map).fillna(999))

    # display name mapping
    df["algo"] = df["algo"].replace(ALGO_RENAME_MAP)
