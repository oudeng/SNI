...
def main():
    ...
    ap.add_argument("--include-algos", type=str, default=None, help="Comma-separated algo filter (filters matrix rows/columns if they match algo names).")
    args = ap.parse_args()
    ...
    # --- algo filter: restrict matrix to matching rows/columns ---
    if args.include_algos:
        _algo_list = [a.strip() for a in args.include_algos.split(",") if a.strip()]
        _keep = [c for c in D.columns if c in _algo_list]
        if _keep:
            D = D.loc[D.index.isin(_keep), _keep]
