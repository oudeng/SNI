...
def plot_overall_bar(
    ...
    ax = fig.add_subplot(111)
    ax.bar(g.index.astype(str).tolist(), g.values.astype(float))
    ax.set_ylabel(base)
...
def plot_taskwise_bars(
    ...
    for t in tasks:
        ...
        ax = fig.add_subplot(111)
        ax.bar(sub["algo"].astype(str).tolist(), sub[metric_mean_col].astype(float).values)
        ax.set_ylabel(base)
...
def main() -> None:
    ...
    if not metrics_mean_cols:
        raise ValueError("No *_mean metric columns found in input CSV(s).")
